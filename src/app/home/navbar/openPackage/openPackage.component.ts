...
export class OpenPackageComponent implements OnInit {
  ...
  loadAllPackage() {
    this.apiService.getAvailablePackage$()
      .subscribe((next) => {
        setTimeout(() => {
          next[0].forEach((element: any) => {
            ...
            classArray.forEach(element => {
              newData.class += this.translateService.instant('gameBase.cardClassName_' + element) + ' ';
            });
  ...
  open(item: basePackageMode) {
    const modalRef = this.modalService.open(ModalOpenPackageComponent, { size: 'lg', backdrop: 'static', animation: true, centered: true, keyboard: false, });
